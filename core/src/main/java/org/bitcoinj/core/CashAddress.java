...
public class CashAddress extends Address {
     ...
     */
    private CashAddress(NetworkParameters params, boolean p2sh, byte[] hash160) throws AddressFormatException {
    ...
    public static CashAddress fromPubKeyHash(NetworkParameters params, byte[] hash160) throws AddressFormatException {
        return new CashAddress(params, false, hash160);
    }
    ...
    public static CashAddress fromScriptHash(NetworkParameters params, byte[] hash160) throws AddressFormatException {
        return new CashAddress(params, true, hash160);
    }
     ...
     */
    public static CashAddress fromBase58(@Nullable NetworkParameters params, String base58)
        ...
        if (params == null) {
            for (NetworkParameters p : Networks.get()) {
                if (version == p.getAddressHeader())
                    return new CashAddress(p, false, bytes);
                else if (version == p.getP2SHHeader())
                    return new CashAddress(p, true, bytes);
            }
            throw new AddressFormatException.InvalidPrefix("No network found for " + base58);
        } else {
            if (version == params.getAddressHeader())
                return new CashAddress(params, false, bytes);
            else if (version == params.getP2SHHeader())
                return new CashAddress(params, true, bytes);
            throw new AddressFormatException.WrongNetwork(version);
        }
    }

    public static CashAddress fromCashAddr(@Nullable NetworkParameters params, String cashAddr) throws AddressFormatException {
        String base58 = AddressConverter.toLegacyAddress(cashAddr);
        return fromBase58(params, base58);
    }
    ...
    public String toString() {
        return AddressConverter.toCashAddress(toBase58());
    }
